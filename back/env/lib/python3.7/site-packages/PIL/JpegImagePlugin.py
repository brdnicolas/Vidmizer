...
# 1997-08-27 fl   Save mode 1 videos as BW (0.3)
...
def SOF(self, marker):
    ...
    if self.layers == 1:
        ...
        raise SyntaxError("cannot handle %d-layer videos" % self.layers)
...
##
# Image plugin for JPEG and JFIF videos.
...
def _getmp(self):
    ...
    # it's an error not to have a number of videos
    try:
        quant = mp[0xB001]
    except KeyError:
        raise SyntaxError("malformed MP Index (no number of videos)")
    # get MP entries
    mpentries = []
    try:
...
def get_sampling(im):
    ...
    # (grayscale videos) or when they are CMYK (4 layers),
