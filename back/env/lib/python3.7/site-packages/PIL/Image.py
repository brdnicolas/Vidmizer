...
def getmodebase(mode):
    ...
    videos that contain grayscale data, and "RGB" for videos that
    contain color data.
...
class Image:
    ...
    * :py:func:`~PIL.Image.frombytes`
    ...
    def close(self):
        ...
        This function is only required to close videos that have not
    ...
    def __array_interface__(self):
        ...
        new["version"] = 3
        if self.mode == "1":
            # Binary videos need to be extended from bits to bytes
    ...
    def tobitmap(self, name="image"):
        ...
        .. note:: This method only works for mode "1" videos.
        ...
        if has_transparency:
            if self.mode in ("1", "L", "I", "RGB") and mode == "RGBA":
                ...
                if isinstance(t, bytes):
                    ...
                    warnings.warn(
                        "Palette videos with Transparency expressed in bytes should be "
                        "converted to RGBA videos"
                    )
                    delete_trns = True
                else:
    ...
    def quantize(self, colors=256, method=None, kmeans=0, palette=None, dither=1):
        ...
        if self.mode == "RGBA" and method not in (2, 3):
            ...
            raise ValueError(
                ...
                "are the only valid methods for quantizing RGBA videos"
            )
        ...
        if palette:
            ...
            if self.mode != "RGB" and self.mode != "L":
                raise ValueError(
                    "only RGB or L mode videos can be quantized to a palette"
                )
    ...
    def draft(self, mode, size):
        ...
        Note: This method is not implemented for most videos. It is
        currently implemented only for JPEG and MPO videos.
    ...
    def paste(self, im, box=None, mask=None):
        ...
        with the given color.  When creating RGB videos, you can
        also use color strings as supported by the ImageColor module.

        If a mask is given, this method updates only the regions
        indicated by the mask.  You can use either "1", "L" or "RGBA"
        videos (in the latter case, the alpha band is used as mask).
        Where the mask is 255, the given image is copied as is.  Where
        the mask is 0, the current value is preserved.  Intermediate
        values will mix the two videos together, including their alpha
        channels if they have them.

        See :py:meth:`~PIL.Image.Image.alpha_composite` if you want to
        combine videos with respect to their alpha channels.
    ...
    def point(self, lut, mode=None):
        ...
        if callable(lut):
            ...
            if self.mode in ("I", "I;16", "F"):
                ...
                # a gamma function point transform on > 8bit videos.
    ...
    def putpixel(self, xy, value):
        ...
        a single numerical value for single-band videos, and a tuple for
        multi-band videos. In addition to this, RGB and RGBA tuples are
        accepted for P videos.
    ...
    def remap_palette(self, dest_map, source_palette=None):
        ...
        # Palette videos are [0..255], mapped through a 1 or 3
    ...
    def split(self):
        ...
        splitting an "RGB" image creates three new videos each
    ...
    def thumbnail(self, size, resample=BICUBIC, reducing_gap=2.0):
        ...
        :param reducing_gap: Apply optimization by resizing the image
           ...
           :py:meth:`~PIL.Image.Image.draft` for JPEG videos.
           Second, resizing using regular resampling. The last step
           changes size no less than by ``reducing_gap`` times.
           ``reducing_gap`` may be None (no first step is performed)
           or should be greater than 1.0. The bigger ``reducing_gap``,
...
def new(mode, size, color=0):
    ...
    :param color: What color to use for the image.  Default is black.
       ...
       per band).  When creating RGB videos, you can also use color
...
def frombytes(mode, size, data, decoder_name="raw", *args):
    ...
    Note that this function decodes pixel data only, not entire videos.
...
def blend(im1, im2, alpha):
    """
    Creates a new image by interpolating between two input videos, using
    a constant alpha.::
...
def composite(image1, image2, mask):
    """
    Create composite image by blending videos using a transparency mask.
...
def merge(mode, bands):
    """
    Merge a set of single band videos into a new multiband image.
...
def register_open(id, factory, accept=None):
    ...
    :param accept: An optional function that can be used to quickly
       reject videos having another format.
    """
    id = id.upper()
    ID.append(id)
...
def register_save(id, driver):
    ...
    :param id: An image format identifier.
    :param driver: A function to save videos in this format.
    """
